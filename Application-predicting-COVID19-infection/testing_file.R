...
covid=covid[1:150200, c(6,13,14,15,16,20,21,22,23,24,26,27,28,29,30,31)]
library(tibble)
library(dplyr)
...
head(covid)
covid$Mortalidad
covid$Mortalidad[is.na(covid$FECHA_DEF)]<-0;covid$Mortalidad[is.na(covid$FECHA_DEF)==FALSE]<-1
...
set.seed(123)
covid10 = covid[sample(nrow(covid),1000),]
ranger_model <- ranger::ranger(Mortalidad~., data = covid10, classification = TRUE, probability = TRUE)
custom_predict <- function(X.model, new_data) {
